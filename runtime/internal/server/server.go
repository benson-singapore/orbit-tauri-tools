	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		defer func() {
			if rec := recover(); rec != nil {
				writeJSON(w, http.StatusInternalServerError, errorBody(fmt.Sprintf("internal error: %v", rec)))
			}
		}()
...
func (s *Server) routes() {
	...
	s.mux.HandleFunc("/v1/youtube/play", s.handleYouTubePlay)
	s.mux.HandleFunc("/v2/runtime/", s.handleRuntimeV2)
	s.mux.HandleFunc("/v2/plugins/", s.handlePluginVariablesV2)
