...
<p align="center">
  <strong>本地优先的多形态桌面阅读器</strong><br/>
  在同一应用中统一浏览文章、漫画、视频、图集与社交短内容
</p>

<p align="center">
  <a href="https://github.com/benson-singapore/orbit-tauri-tools/releases"><img src="https://img.shields.io/github/v/release/benson-singapore/orbit-tauri-tools?style=flat-square&label=release" alt="Release" /></a>
  <img src="https://img.shields.io/badge/platform-macOS-black?style=flat-square" alt="Platform" />
  <img src="https://img.shields.io/badge/Tauri-2-blue?style=flat-square" alt="Tauri" />
  <img src="https://img.shields.io/badge/license-OPNCL-orange?style=flat-square" alt="License" />
</p>
...
<p align="center">
  ...
  <a href="#核心特性">核心特性</a> ·
  <a href="#系统架构">系统架构</a> ·
  <a href="https://github.com/benson-singapore/orbit-tauri-tools/releases">下载安装包</a>
</p>

---

<p align="center">
  <img src="https://orbit-plugins-resource.pages.dev/orbit-screenshots/iShot_2026-06-27_17.30.11.jpg" alt="Orbit 应用界面预览" width="100%" />
</p>

<p align="center"><em>统一订阅 · 多布局阅读 · 本地 Runtime 驱动</em></p>

---

## 项目简介

Orbit 是一个面向多内容形态的**本地优先**桌面阅读器。它把分散在不同来源、不同格式的内容，收敛到一套连贯的本地阅读体验中：

- 用**插件**接入不同内容源（RSS、WASM、`.orbit` 包等）
- 用**统一界面**管理订阅、阅读、播放与历史记录
- 用**本地 Runtime** 提供更稳定的解析、缓存与数据存储能力

技术栈：**Tauri 2 + React + Go Runtime + SQLite**。当前首发平台为 **macOS**。
...
|------|------|
| 插件市场 | 浏览、安装、更新官方插件 |
| RSS 导入 | 支持自定义 RSS / Atom 订阅源 |
| WASM / `.orbit` | 支持 WASM 插件与 `.orbit` 插件包（含 URL 远程导入） |
| 插件分组 | 侧栏折叠、排序、自定义分组 |
| 体验模式 | 安全级 / 完整级，控制成人向内容展示 |
| CF 会话验证 | 遇到 Cloudflare / Captcha / 403 时可引导浏览器验证并自动重试 |
...
| 文章阅读器 | 字体调节、内容宽度调节、代码高亮、图片代理、点击预览 |
| 漫画 / 小说 | 章节目录（正序/倒序）、连续翻页、章节流预加载、编码兼容 |
| 视频播放 | HLS 播放、YouTube 嵌入中继、视频墙同播 |
| 音频与 TTS | 音量 / 倍速、封面代理、文章 TTS 朗读、动态音轨解析 |
...
| 桌面壳 | `app/src-tauri/` | 窗口管理、Sidecar 拉起、系统 API、浏览器会话验证 |
...
| `make icons` | 从 `app/src-tauri/icons/icon.png` 重新生成图标 |
...
推送 `v*` 格式的 tag（如 `v1.1.4`）会触发 [`.github/workflows/release.yml`](.github/workflows/release.yml)，自动并行构建并发布：
...
插件可通过应用内**插件市场**安装与更新，也可手动放置到用户插件目录。面对 Cloudflare 等防护站点时，插件可声明浏览器会话验证，由应用引导完成验证后自动重试请求。
...
| 平台 | 默认路径 |
|------|----------|
| macOS 数据库 | `~/Library/Application Support/Orbit Reader/orbit.db` |
...
在既有能力之上，本版本重点补齐 **Cloudflare 人机验证**、增强 **音频 / TTS**，并完善阅读、漫画、收藏与体验模式等日常体验。
...
- Cloudflare / Captcha / 403 拦截时可自动拉起浏览器会话验证，并持久化 Cookie / User-Agent 后重试
- 音频播放增强（音量、倍速、封面代理、动态音轨解析）与文章 TTS 朗读
- 阅读内容宽度调节；漫画章节流与 CDN 图像代理优化
- 文章收藏、体验模式快捷键解锁（`⌘⇧L`）
- YouTube 嵌入中继，改善 Tauri 环境下的视频播放
...
| 视频 / 音频 | hls.js、YouTube 嵌入、APlayer |

---

## 许可证

本项目采用 [Orbit 个人非商业使用许可协议 (OPNCL) v1.0](./LICENSE)。

**简要说明：**

- 允许个人出于学习、研究、体验等非商业目的使用、研究及本地修改本软件
- **禁止**任何商业使用，包括但不限于出售、收费分发、广告变现、企业内商用
- **禁止**将修改后的版本用于牟利；再分享时须附带相同许可条款
- 如需商业授权，请事先联系著作权人取得书面许可

本软件所包含的第三方依赖，仍遵循其各自的开源许可协议。
